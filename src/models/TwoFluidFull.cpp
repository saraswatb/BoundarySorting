...
TwoFluidFull::TwoFluidFull(unsigned LX, unsigned LY, unsigned BC)
  : Model(LX, LY, BC, BC==0 ? GridType::Periodic : GridType::Layer)
{}
TwoFluidFull::TwoFluidFull(unsigned LX, unsigned LY, unsigned BC, GridType Type)
  : Model(LX, LY, BC, Type)
{}

void TwoFluidFull::Initialize()
{
  ...
  //gn1_tmp.SetSize(LX, LY, Type);
  //gn1.SetSize(LX, LY, Type);  
  //gn2.SetSize(LX, LY, Type);
  //gn2_tmp.SetSize(LX, LY, Type);
  //logn1.SetSize(LX, LY, Type);
  */
  ...
  /*
  //QNxx1.SetSize(LX, LY, Type);
  //QNyx1.SetSize(LX, LY, Type);
  //QNxx2.SetSize(LX, LY, Type);
  //QNyx2.SetSize(LX, LY, Type);
  */
  ...
  //additional fields for different settings
  if (thermoAnchor){
  ...
  if (dropletMode){
    masterPhaseField.SetSize(LX, LY, Type);  
  }
...
void TwoFluidFull::ConfigureAtNode(unsigned k)
{
  ...
  double dubLX = LX, dubLY = LY, xtemp=x-dubLX/2, ytemp=y-dubLY/2;  
  
  //double ux1_tilde, uy1_tilde;
  if (dropletMode){
    masterPhaseField[k] = 1;
  }
  ...
  else if(init_config == "HalfAndHalf")
  {
    if (x<LX/2){
      nematicOrder2 = 0;
      conc1 = 1;
  ...
  else if(init_config == "NoisyUnif" || init_config == "ChannelTest2" || init_config == "ChannelTest1"){
    conc1 = 1+0.04*(random_real()-0.5);
    conc2 = 1+0.04*(random_real()-0.5);
  }
  else if(init_config == "Noise"){
    conc1 = 0.5+0.9*(random_real()-0.5);
    conc2 = 0.5+0.9*(random_real()-0.5);
  }
  else if(init_config == "BlobMid"){
    double rtemp = sqrt(xtemp*xtemp+ytemp*ytemp);
    if (rtemp  <= radius ){ //blob in centre
      conc1 = level;
      conc2 = 1-level;  
    }
    else if (rtemp <=2*radius){ //smooth transition
      conc1 = (1-level)+(2-rtemp/radius)*(2*level-1);
      conc2 = 1-conc1;
    }
    else{ //background
      conc1 = 1-level;
    ...
    nematicOrder1 = Snem1*sqrt(conc1);  
    nematicOrder2 = Snem2*sqrt(conc2);  
  }
  else if(init_config == "SameSignBlobs"){ //make sure 2*radius divides LX, LY
    xtemp = x % (int) (2*radius); ytemp = y % (int) (2*radius);

    if ( (xtemp-radius)*(xtemp-radius)+(ytemp-radius)*(ytemp-radius) <= radius*radius ){
      conc1 = 0.55;
      conc2 = 0.45;
    }
    else{
  ...
  else if(init_config == "AlternatingStripes"){ //make sure 2*radius divides LX
    xtemp = x % (int) (2*radius); 
  ...
  else if(init_config == "LGInterface"){
    //we define an interface \phi_int = 0.5 + level*tanh( (x-LX/2)/radius )
    conc1 = 2*(0.5+level*tanh(ytemp/radius));
    conc2 = 2*(0.5-level*tanh(ytemp/radius));
    nematicOrder1 = (0.5+level*tanh((y-dubLY/2)/radius));
    nematicOrder2 = (0.5-level*tanh((y-dubLY/2)/radius));
  }
  else if(init_config == "UnstableInterface"){
    //we define an interface \phi_int = 0.5 + level*tanh( (y-LY/2)/radius )
    conc1 = 2*(0.5+level*tanh(ytemp/radius));
    conc2 = 2*(0.5-level*tanh(ytemp/radius));
    nematicOrder1 = (0.5+level*tanh((y-dubLY/2)/radius));
    nematicOrder2 = (0.5-level*tanh((y-dubLY/2)/radius));
  }
  else if(init_config == "UnevenConcs"){
    conc1 = level*(1+0.04*(random_real()-0.5));
    conc2 = (1-level)*(1+0.04*(random_real()-0.5));
    //so we'll now have different densities of particles
  }
  else if(init_config == "Circles"){ 
    ...
    if (!dropletMode) return;
      ...
      if (rad_test  > 1.1*radius ){
        nematicOrder1 = 0.0;
        nematicOrder2 = 0.0;
        masterPhaseField[k] = 0;      
      }
      else if (rad_test  > radius ){
        nematicOrder1 = 0.0;
        nematicOrder2 = 0.0;
        masterPhaseField[k] = 0.5;      
      }
      else {
        masterPhaseField[k] = 1;
        nematicOrder1 = 1.0;
    ...
    if (!dropletMode) return;
      ...
      if (xtemp>0){ //ellipse side
        rad_test = sqrt(xtemp*xtemp/(1+level)/(1+level)+ytemp*ytemp);
      }
  ...
  else if(init_config == "NucleateNoise"){
    //try nucleating without Q noise (hence large S) at certain sites
    xtemp = x % ((int) (2*radius)) - (int) radius; 
    ytemp = y % ((int) (2*radius)) - (int) radius; 
    conc1 = 1; conc2 = 1;
    if (abs(xtemp)>0.5*radius || abs(ytemp)>0.5*radius){
      nematicOrder1 = 0.25;
    }
  }
  else if(init_config == "HoleyDrop"){
    if ( (xtemp*xtemp+ ytemp*ytemp <= radius*radius) && (xtemp*xtemp+ ytemp*ytemp >= level*level) ){ //level radius hole in a radius radius drop
      conc1 = 0.75;
      conc2 = 0.25;
    }
    else{
  ...
  //different nematic configurations
  else if(init_config == "Hedgehog" || init_config == "Vortex"){
    // do nothing 
    conc1 = 1; conc2 = 1;
    /*
    if (xtemp*xtemp + ytemp*ytemp < radius*radius){
      ...
      conc1 = (dubLX*dubLY - M_PI*radius*radius*level)/(dubLX*dubLY - M_PI*radius*radius);
    }
    */
  }
  ...
  else if(init_config == "TwoHedgehogs"){
    // do nothing 
    conc1 = 1; conc2 = 1;
    /*
    if (xtemp*xtemp + ytemp*ytemp < radius*radius){
    ...
    */
  ...
  else if(init_config == "PerfectAnchoringTest"){    
    int xlatch = (xtemp>0)?1:-1;
    int ylatch = (ytemp>0)?1:-1;
    if (x < LX/2){
      xtemp = xtemp+dubLX/4;
    }
    else{
    ...
    conc1 = 0.5 -  ylatch*xlatch*level*tanh(xtemp/radius);
    conc2 = 1 - conc1;
  }  
  else if(init_config == "PerfectAnchoringTest2"){    
    int xlatch = (xtemp>0)?1:-1;    
    if (x < LX/2){
      xtemp = xtemp+dubLX/4;
    }
    ...
    conc1 = 0.5 -  xlatch*level*tanh((xtemp-radius)/10.0);
    conc2 = 1 - conc1;
  }
  else if(init_config == "PlusHalfDefect" || init_config == "MinusHalfDefect"){
    conc1 = 1; conc2 = 1;
  }
  else if(init_config == "CoalignmentTest"){
    conc1 = 1; conc2 = 1;
  }
  else
    throw error_msg("error: initial configuration '", init_config, "' unknown.");

  theta1   = noise*M_PI*(random_real() - .5) + angle; 
  theta2   = angle+ noise*M_PI*(random_real() - .5);
  
  //defining theta fields for different nematic configuations
  if      (init_config == "Hedgehog"){    
    theta1 = atan2(ytemp, xtemp) + (zeta1>0? M_PI/2: 0) + noise*M_PI*(random_real() - .5);
    theta2 = atan2(ytemp, xtemp) + (zeta2>0? M_PI/2: 0) + noise*M_PI*(random_real() - .5);
  }
  else if (init_config == "Vortex" || init_config =="HoleyDrop" || init_config == "BlobMid" || init_config == "Circles"){    
    theta1 = atan2(ytemp, xtemp) + M_PI/2  + noise*M_PI*(random_real() - .5);
    theta2 = atan2(ytemp, xtemp) + M_PI/2  + noise*M_PI*(random_real() - .5);
  }
  else if (init_config == "TwoHedgehogs"){
    double xtemp2 = fmod(xtemp+dubLX/8, dubLX);
    xtemp = fmod(xtemp - dubLX/8, dubLX);
    theta1 = atan2(ytemp, xtemp) + atan2(ytemp, xtemp2) + (zeta1>0? 0: 0) + noise*M_PI*(random_real() - .5);
    theta2 = atan2(ytemp, xtemp) + atan2(ytemp, xtemp2) + (zeta2>0? 0: 0) + noise*M_PI*(random_real() - .5);
  }
  else if (init_config == "Aster"){
    ...
    theta1 = angle; theta2 = angle;    
  }
  else if (init_config == "ChannelTest2"){
    theta1 = angle + 0.1*sin(y*M_PI/LY) + noise*M_PI*(random_real() - .5);
    theta2 = theta1;
  }
  else if (init_config == "ChannelTest1"){
    theta1 = angle*cos(y*M_PI/LY) + noise*M_PI*(random_real() - .5);
    theta2 = theta1;
  }
  else if (init_config == "PlusHalfDefect"){
    //initialize a defect at the centre of the domain    
    double rtemp = sqrt(xtemp*xtemp+ytemp*ytemp);
    if ((rtemp < radius) && (rtemp > 1)){
      double phi_def = acos(xtemp/rtemp);
      if (ytemp < 0)  phi_def = -phi_def;
      theta1 = angle + phi_def/2.0;      
    } 
  }
  else if (init_config == "MinusHalfDefect"){
    //initialize a defect at the centre of the domain    
    double rtemp = sqrt(xtemp*xtemp+ytemp*ytemp);
    if ((rtemp < radius) && (rtemp > 1)){
      double phi_def = acos(xtemp/rtemp);
  ...
  /*
  if (fix_Q1){
    theta1 = sin(2*M_PI*x*level/(LX*1.0));
  }
  */
  //printf("coord is %u and %u, coordtemp is %f and %f, theta is %f\n",y,x,ytemp-LY/2,xtemp-LX/2,theta);
  //nematicOrder += noise*random_real();

  QQxx1[k] = nematicOrder1*(cos(2*theta1));
  QQyx1[k] = nematicOrder1*(sin(2*theta1));

  QQxx2[k] = nematicOrder2*(cos(2*theta2));
  QQyx2[k] = nematicOrder2*(sin(2*theta2));
  ...
  if (fix_v1){//impose a fixed flow for debugging
    ...
    uy1[k] = level*sin(2*M_PI*y/radius); //shear flow
    ux1[k] = 0;
  }
...
LBNode TwoFluidFull::CorrectLBAtNode(LBNode fb, unsigned k){
  ...
  fb[1] = fb[1] + 1./4.*KK*(dxp2-dyp2);
  fb[2] = fb[2] + 1./4.*KK*(dxp2-dyp2);
  fb[3] = fb[3] - 1./4.*KK*(dxp2-dyp2);
  fb[4] = fb[4] - 1./4.*KK*(dxp2-dyp2);
  fb[5] = fb[5] + 1./4.*KK*(dxpdyp);
  fb[6] = fb[6] + 1./4.*KK*(dxpdyp);
  fb[7] = fb[7] - 1./4.*KK*(dxpdyp);
  fb[8] = fb[8] - 1./4.*KK*(dxpdyp);
...
void TwoFluidFull::UpdateNemQuantitiesAtNode(unsigned k, bool first){
  ...
  /* S_nem spatial modifiers. Pretty useless.
  double Snem1_mod= 1.0;
  double Snem2_mod= 1.0;
  * Write a test case to see if the active nematic clusters if nematic activity is favoured.
  * We shall keep this section deleted as a comment later, since it makes the code less cluttered.
  *  
  const unsigned x = GetXPosition(k);
  const unsigned y = GetYPosition(k);
  if (x>=LX/2-30 && x<=LX/2 + 30 && y>=LY/2-30 && y<=LY/2+30){ //define a box inside where Snem1 is lower
    Snem1 = 0.30;
  }
  *

  if (init_config == "NucleateNoise"){    
    const unsigned x = GetXPosition(k);
    const unsigned y = GetYPosition(k);
    double xtemp = x % ((int) (2*radius)) - (int) radius; 
    double ytemp = y % ((int) (2*radius)) - (int) radius; 
    if (abs(xtemp)<0.5*radius && abs(ytemp)<0.5*radius){
      Snem1_mod = 0.25;
    }
  }
  */
  ...
  const double term1 = Snem1*Snem1 - Qxx1*Qxx1 - Qyx1*Qyx1; //*p 
   
  double Hxx1 = CC1*term1*Qxx1 + LL1*del2Qxx1;
  double Hyx1 = CC1*term1*Qyx1 + LL1*del2Qyx1;
  ...
  if (thermoAnchor){ //calculate and update thermodynamic anchoring terms for Hxx and Hxy    
    const double dxp = derivX(phi, d, sB);
    const double dyp = derivY(phi, d, sB);
    Hxx1 = Hxx1 + L_thermo*(dxp*dxp-dyp*dyp)/2;
    Hyx1 = Hyx1 + L_thermo*dxp*dyp;    
  }

  if (LL12 != 0 || K12_NR != 0){ //Q1 coupling to Q2
    //Hxx1 = Hxx1 - (LL12+K12_NR)*del2Qxx2;
    //Hyx1 = Hyx1 - (LL12+K12_NR)*del2Qyx2;
    Hxx1 = Hxx1 + (LL12+K12_NR)*Qxx2*p*(1-p); //
    Hyx1 = Hyx1 + (LL12+K12_NR)*Qyx2*p*(1-p); //
  ...
  const double term2 = Snem2*Snem2 - Qxx2*Qxx2 - Qyx2*Qyx2;  //*(1-p)
  double Hxx2 = CC2*term2*Qxx2 + LL2*del2Qxx2;
  double Hyx2 = CC2*term2*Qyx2 + LL2*del2Qyx2;
  
  if (LL12 != 0 || K21_NR != 0){ //Q2 coupling to Q1
    //Hxx2 = Hxx2 - (LL12+K21_NR)*del2Qxx1;
    //Hyx2 = Hyx2 - (LL12+K21_NR)*del2Qyx1;
    Hxx2 = Hxx2 + (LL12+K21_NR)*Qxx1*p*(1-p);
    Hyx2 = Hyx2 + (LL12+K21_NR)*Qyx1*p*(1-p);
  }
  ...
  //list of configurations with activity modification
  double zetaMod = 1;
  if (init_config == "Hedgehog"){
    const double x = (double) GetXPosition(k);
    const double y = (double) GetYPosition(k);
    const double r = sqrt( (x-LX/2.0)*(x-LX/2.0)+(y-LY/2.0)*(y-LY/2.0) );

    zetaMod = 4*r/LX;
  }
  //remove flag
  //zetaMod = 1; //(p>0.50? 1:0);


  //these quantities are calculated BEFORE the first nematic update step  
  const double sigmaB1 = (backflow_on? .5*CC1*term1*term1 : 0) - (wallEdge ? FE_anchoring: 0); 
  const double sigmaF1 =  - (preinit_flag ? 0 : zeta1*zetaMod*Qxx1) 
                          + (backflow_on? 2*xi1*( (Qxx1*Qxx1-1)*Hxx1 + Qxx1*Qyx1*Hyx1 ) : 0)                          
                          + (backflow_on? LL1*(dyQxx1*dyQxx1+dyQyx1*dyQyx1-dxQxx1*dxQxx1-dxQyx1*dxQyx1) : 0)  ;
                          //- (backflow_on? 2*(LL12+K12_NR)*(dyQxx1*dyQxx2+dyQyx1*dyQyx2-dxQxx1*dxQxx2-dxQyx1*dxQyx2) : 0) ;
  const double sigmaS1 =  - (preinit_flag ? 0 : zeta1*zetaMod*Qyx1)
                          + (backflow_on? 2*xi1*(Qyx1*Qxx1*Hxx1 + (Qyx1*Qyx1-1)*Hyx1) : 0)
                          - (backflow_on? 2*LL1*(dxQxx1*dyQxx1+dxQyx1*dyQyx1) : 0) ;
                          // + (backflow_on? 2*(LL12+K12_NR)*(dxQxx1*dyQxx2+dxQyx1*dyQyx2) : 0) ;                         
  ...
  const double sigmaF2 =  - (preinit_flag ? 0 : zeta2*Qxx2) 
                          ...
                          //- (backflow_on? 2*(LL12+K21_NR)*(dyQxx1*dyQxx2+dyQyx1*dyQyx2-dxQxx1*dxQxx2-dxQyx1*dxQyx2) : 0) ;                
  const double sigmaS2 =  - (preinit_flag? 0 : zeta2*Qyx2)
                          + (backflow_on? 2*xi2*(Qyx2*Qxx2*Hxx2 + (Qyx2*Qyx2-1)*Hyx2) : 0)
                          - (backflow_on? 2*LL2*(dxQxx2*dyQxx2+dxQyx2*dyQyx2) : 0) ;
                          //+ (backflow_on? 2*(LL12+K21_NR)*(dxQxx1*dyQxx2+dxQyx1*dyQyx2) : 0) ;                             
  ...
  //const double dFdphi_CrossTerm  = -2*LL12*(1-2*p)*(Qxx1*Qxx2 + Qyx1*Qyx2);
  BulkFE_LDG_12[k] = (backflow_on? dFdphi : 0);// + dFdphi_CrossTerm; 

}
void TwoFluidFull::UpdateNemQuantities(bool first){  
  // for reduction (+:sum)
  #pragma omp parallel for num_threads(nthreads) if(nthreads)
  for(unsigned k=0; k<DomainSize; k++)
  {
    if (dropletMode && (masterPhaseField[k] != 1)) continue;
    UpdateNemQuantitiesAtNode(k, first);
  }
}

void TwoFluidFull::UpdateFluidQuantitiesAtNode_p1(unsigned k){  
  //const auto& d = get_neighbours(k);
  const auto& f = ff[k];
  const auto& g2 = gg2[k];
  ...
  //bug_fixing explicitly - hard stops of phi going out of bounds
  const double nTOL = 0.10;
  //double extra_particles = 0;
  if (nn1/nnC <= nTOL){ //too few particles
    //extra_particles = -nn1+ nnC*nTOL;
    nn1 = nTOL*nnC;    
  } 
  else if (nn1/nnC >= 1-nTOL){ //too many particles
    //extra_particles = nn1- nnC*(1-nTOL);
    nn1 = (1-nTOL)*nnC;
  }  
  //right now, fix extra particles at runtime checks
...
void TwoFluidFull::EvaluatePhiQuantities(unsigned k)
{
  const auto& d = get_neighbours(k); 
...
void TwoFluidFull::UpdateFluidQuantitiesAtNode_p2(unsigned k){  
  const auto& d = get_neighbours(k); 
  const auto& f = ff[k];
  const auto& g1 = gg1[k];

  const double nnC = rho1+rho2; //nC[k];
  const double p = phi[k];  
  ...
  double fr1_mod = 1.0;
  /* Implementation of regions of different frictions
   * in a very inelegant way.
   *
  const unsigned x = GetXPosition(k);  
  if (fric_track_test){ 
    //fr1_mod = 1+ level*abs(sin(2*M_PI*x*4/(LX*1.0)));
    fr1_mod = ((x/(LX/4))%2 == 0)?1:5;
  }
  */
  

  const double AA_LBF_C = isGuo? 0.5 : p*tau1 + (1-p)*tau2; //that is, for the combined fluid
  //const double p_toggle = phi_FreeE ? p : 0.5;

  double vxC = fix_vc? uxC[k] : (f[1] - f[2] + f[5] - f[6] - f[7] + f[8] + AA_LBF_C*(Fx1+Fx2))/(nnC + AA_LBF_C*(p*friction1*fr1_mod + (1-p)*friction2));
  double vyC = fix_vc? uyC[k] : (f[3] - f[4] + f[5] - f[6] + f[7] - f[8] + AA_LBF_C*(Fy1+Fy2))/(nnC + AA_LBF_C*(p*friction1*fr1_mod + (1-p)*friction2));
  ...
  //const double dxnnC = derivX(nC, d, sB);
  //const double dynnC = derivY(nC, d, sB);
  const double d2dxPhi = laplacian(dxPPhi, d, sD);
  const double d2dyPhi = laplacian(dyPPhi, d, sD);

  /** Implementation of interface term */
  double phi_eqb = 0.5; double dyphi_correction =0; 
  if(init_config == "LGInterface"){
    //we define an interface \phi_int = 0.5 + level*tanh( (x-LX/2)/radius )
    const unsigned y = GetYPosition(k);
    const double tanh_factor = tanh((y-LY/2.0)/radius);
    phi_eqb = 0.5+level*tanh_factor;
    dyphi_correction = level*(1-tanh_factor*tanh_factor)/radius;    
  }
  else if (init_config == "UnevenConcs"){
    phi_eqb = level;
  }

  /* Landau free energy for the bulk 
   */   
  //double Lan_FreeEn    = 2*landA*(p-phi_eqb) + 4*landB*(p-phi_eqb)*(p-phi_eqb)*(p-phi_eqb);
  double derLan_FreeEn = 2*landA+ 12*landB*(p-phi_eqb)*(p-phi_eqb); // (dropletMode?(masterPhaseField[k]==1?1:10):1)            
  //
 
  const double F_LG_x  = nnC*derLan_FreeEn*(dxPhi) - KK*d2dxPhi; //dxnnC*Lan_FreeEn + 
  const double F_LG_y  = nnC*derLan_FreeEn*(dyPhi-dyphi_correction) - KK*d2dyPhi; //dynnC*Lan_FreeEn + 

  const double F_LdG_x = derivX(BulkFE_LDG_12, d, sB);
  const double F_LdG_y = derivY(BulkFE_LDG_12, d, sB);
  
  //check if this needs changing
  double rel_fric_mod = 1; //dropletMode ? (masterPhaseField[k] == 1 ? 1 : 5) : 1;

  const double sfX_stub =  rel_fric*rel_fric_mod*vxC  - (1-p)*(F_LG_x + F_LdG_x); //(dropletMode?(masterPhaseField[k]==1?1:4):1)*
  const double sfY_stub =  rel_fric*rel_fric_mod*vyC  - (1-p)*(F_LG_y + F_LdG_y); //(dropletMode?(masterPhaseField[k]==1?1:4):1)*

  const double AA_LBF = isGuo? 0.5 : tau1;
  
  
  //determine velocities self-consistently
  //const double vx1 = fix_v1? ux1[k] : (g1[1] - g1[2] + g1[5] - g1[6] - g1[7] + g1[8] + AA_LBF*(sfX_stub + FFx1[k]/p))/(nnC + AA_LBF*rel_fric + AA_LBF*friction1);
  //const double vy1 = fix_v1? uy1[k] : (g1[3] - g1[4] + g1[5] - g1[6] + g1[7] - g1[8] + AA_LBF*(sfY_stub + FFy1[k]/p))/(nnC + AA_LBF*rel_fric + AA_LBF*friction1);
  //
  double vx1 = fix_v1? ux1[k] : ( g1[1]/phi_old[d[opp(1)]] - g1[2]/phi_old[d[opp(2)]] 
          ...
          + g1[8]/phi_old[d[opp(8)]]  +AA_LBF*(sfX_stub + FFx1[k]/p))/(nnC + AA_LBF*rel_fric*rel_fric_mod + AA_LBF*friction1*fr1_mod);
  double vy1 = fix_v1? uy1[k] : ( g1[3]/phi_old[d[opp(3)]] - g1[4]/phi_old[d[opp(4)]] 
          + g1[5]/phi_old[d[opp(5)]] - g1[6]/phi_old[d[opp(6)]] + g1[7]/phi_old[d[opp(7)]] 
          - g1[8]/phi_old[d[opp(8)]]  +AA_LBF*(sfY_stub + FFy1[k]/p))/(nnC + AA_LBF*rel_fric*rel_fric_mod + AA_LBF*friction1*fr1_mod);
  ...
  /*
  if (thermoAnchor){
    double d2xp = derivXX(phi, d, sB);
    double d2yp = derivYY(phi, d, sB);
    double dxdyp = derivXY(phi, d, sB);
...
void TwoFluidFull::UpdateFluidQuantities()
{
  // for reduction (+:sum)
  #pragma omp parallel for num_threads(nthreads) if(nthreads)
  for(unsigned k=0; k<DomainSize; ++k)
...
void TwoFluidFull::UpdateNemFieldsAtNode(unsigned k, bool first){
  const auto& d = get_neighbours(k);
  
  if (init_config == "PerfectAnchoringTest" || init_config == "PerfectAnchoringTest2") return;
  ...
  //advection matrices are for Force balance debugging purposes
  advX1[k] = n1[k]*(vx1-ux_old1[k] + vx1*dxux1 + vy1*dyux1);
  advY1[k] = n1[k]*(vy1-uy_old1[k] + vx1*dxuy1 + vy1*dyuy1);  
  advX2[k] = n2[k]*(vx2-ux_old2[k] + vx2*dxux2 + vy2*dyux2);
  advY2[k] = n2[k]*(vy2-uy_old2[k] + vx2*dxuy2 + vy2*dyuy2); 
  ...
  /*
  double crossEl_xx1 = 0, crossEl_yx1 = 0; 
  double crossEl_xx2 = 0, crossEl_yx2 = 0;  
  // cross elasticity contributions
  if (LL12 != 0 || K12_NR != 0 || K21_NR != 0){
    //additional parameters required
    const double p = phi[k];
    const double d2Qxx1 = laplacian(QQxx1, d, sD);
    const double d2Qyx1 = laplacian(QQyx1, d, sD);
    const double d2Qxx2 = laplacian(QQxx2, d, sD);
    const double d2Qyx2 = laplacian(QQyx2, d, sD);
    const double dxp    = dxPPhi[k];
    const double dyp    = dyPPhi[k];

    crossEl_xx1 = -(LL12 + K12_NR)*( p*(1-p)*d2Qxx1 + (1-2*p)*(dxQxx1*dxp + dyQxx1*dyp) );
    crossEl_yx1 = -(LL12 + K12_NR)*( p*(1-p)*d2Qyx1 + (1-2*p)*(dxQyx1*dxp + dyQyx1*dyp) );
    crossEl_xx2 = -(LL12 + K21_NR)*( p*(1-p)*d2Qxx2 + (1-2*p)*(dxQxx2*dxp + dyQxx2*dyp) );
    crossEl_yx2 = -(LL12 + K21_NR)*( p*(1-p)*d2Qyx2 + (1-2*p)*(dxQyx2*dxp + dyQyx2*dyp) );
  ...
  */
  ...
  const double Dxx1       = GammaQ1*Hxx1 - 2*vorticity1*Qyx1- 1*( vx1*dxQxx1 + vy1*dyQxx1 ) // + Qxx1*expansion1
                            + xi1*((Qxx1+1)*(2*dxux1-traceQL1) +2*Qyx1*shear1 -expansion1-Qxx1*expansion1); 
  const double Dyx1       = GammaQ1*Hyx1 + 2*vorticity1*Qxx1- 1*( vx1*dxQyx1 + vy1*dyQyx1 ) //+ expansion1*Qyx1
  ...
  const double Dxx2       = GammaQ2*Hxx2 - 2*vorticity2*Qyx2- 1*( vx2*dxQxx2 + vy2*dyQxx2  ) //+ expansion2*Qxx2
                            + xi2*((Qxx2+1)*(2*dxux2-traceQL2) +2*Qyx2*shear2 -expansion2-Qxx2*expansion2); 
  const double Dyx2       = GammaQ2*Hyx2 + 2*vorticity2*Qxx2- 1*( vx2*dxQyx2 + vy2*dyQyx2  ) //+ expansion2*Qyx2
...
void TwoFluidFull::UpdateNemFields(bool first){
  #pragma omp parallel for num_threads(nthreads) if(nthreads)
  for(unsigned k=0; k<DomainSize; ++k){
    if (dropletMode && (masterPhaseField[k] != 1)) continue;
...
void TwoFluidFull::UpdateFluidFieldsAtNode(unsigned k, bool first)
{
  const auto& d = get_neighbours(k);
  
  const double vx1 = ux1[k]; 
  const double vy1 = uy1[k];
  const double vxC = uxC[k];
  const double vyC = uyC[k];
  const double vx2 = ux2[k];
  const double vy2 = uy2[k];
  const double nn1 = n1[k];
  ...
  const double fr1_mod = 1.0;
  /* Implementation of regions of different frictions
   * in a very inelegant way.
   *
  const unsigned x = GetXPosition(k);
  
  if ((x/20)%2==0){ //(x>=LX/2-30 && x<=LX/2 + 30 && y>=LY/2-30 && y<=LY/2+30){
    fr1_mod = 0.1;
  }
  */

  const double FxC = malvnt_test? level*cos(0.1*my_timer)*cos(2*M_PI/10*GetYPosition(k)) : 
                              FFx1[k] + FFx2[k] - p_toggle*friction1*fr1_mod*vx1 - (1-p_toggle)*friction2*vx2; 
  const double FyC = malvnt_test? 0 : FFy1[k] + FFy2[k]-p_toggle*friction1*fr1_mod*vy1 - (1-p_toggle)*friction2*vy2;                               

  double Gx = p_toggle*(subForceX[k] + FFx1[k]/p_toggle) - friction1*fr1_mod*vx1;
  double Gy = p_toggle*(subForceY[k] + FFy1[k]/p_toggle) - friction1*fr1_mod*vy1;
  ...
  // relaxation time (determines combine viscosity)
  double tau = (p_toggle*tau1+(1-p_toggle)*tau2)*(dropletMode?(masterPhaseField[k]==1?1:100):1);
  //double tau1mod = (masterPhaseField[k]==1?1:4);

  if (viscChange){ //for the Malevanets simulation test, or to help phase separation
    if (malvnt_test){
      tau = (p<0.5) ? 0.626 : 1.25;
    }
    else{
      tau = (p>0.5) ? 2 : 1;
    }
  ...
  if(first)
  {
    for(unsigned v=0; v<lbq; ++v)
    {
      ...
      gg1[k][v] = gg1[k][v] +    (ge1[v]-gg1[k][v])/tau1 +    Si_1;  
      gg2[k][v] = gg2[k][v] +    (ge2[v]-gg2[k][v])/tau1;
    }
...
void TwoFluidFull::UpdateFluidFields(bool first)
{
  #pragma omp parallel for num_threads(nthreads) if(nthreads)
  for(unsigned k=0; k<DomainSize; ++k){
    //if (masterPhaseField[k] == 0) continue;
...
void TwoFluidFull::BoundaryConditionsLB()
{  
  switch(BC)
  {
...
void TwoFluidFull::BoundaryConditionsFields()
{  
  ...
  switch(BC)
  {
    ...
    */
    QQxx1.ApplyDirichletChannel(anchorSign1*wallAnchor);
    QQyx1.ApplyDirichletChannel(0);
    QQxx2.ApplyDirichletChannel(0);
    QQyx2.ApplyDirichletChannel(0);
      break;
    // box
    case 3:
    case 4:
...
void TwoFluidFull::BoundaryConditionsFields2()
{
  switch(BC)
  {
    // pbc without bdry layer (nothing to do)
...
void TwoFluidFull::RuntimeChecks()
{
...
option_list TwoFluidFull::GetOptions()
{
  ...
  config_options.add_options()
    ...
    ("initseed", opt::value<int>(&initseed),
     "initial seed")
    ;
